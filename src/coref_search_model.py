...
class SimilarityModel(object):
    def __init__(self, in_batch_samples, neg_exmpl, device):
        self.in_batch_samples = in_batch_samples
        self.neg_exmpl = neg_exmpl
        self.device = device

    def extract_embeddings(self, outputs: QuestionAnsweringModelOutput, passages_bounds,
                           query_event_starts, query_event_ends):
        # +1 for positive example
        tot_exmpl = self.neg_exmpl + 1
        ...
        query_indices = torch.arange(start=0, end=outputs.query_hidden_states.size(0), step=tot_exmpl)
        ...
        query_hidden_avg = torch.mean(outputs.query_hidden_states.view(self.in_batch_samples, tot_exmpl,
                                                                       outputs.query_hidden_states.shape[1], -1), dim=1)

        query_div_inds = torch.arange(query_hidden_avg.size(0))
        query_start_embeds = outputs.query_hidden_states[query_div_inds, query_event_starts]
        query_end_embeds = outputs.query_hidden_states[query_div_inds, query_event_ends]

        passage_rep = torch.cat((passages_start_embeds, passage_end_embeds), dim=1).view(self.in_batch_samples,
                                                                                         tot_exmpl, -1)
        ...
        return query_rep, passage_rep

    def calc_similarity_loss(self, query_rep, passage_rep, positive_idxs):
        predicted_idxs, softmax_scores = self.predict_softmax(query_rep, passage_rep)
        sim_loss = F.nll_loss(
            softmax_scores,
            torch.tensor(positive_idxs).to(softmax_scores.device),
            reduction="mean",
        )
        return sim_loss, predicted_idxs, positive_idxs
