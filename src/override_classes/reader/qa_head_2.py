...
class WECQuestionAnsweringHead(PredictionHead):
    ...
    def __init__(self, layer_dims: List[int] = [768, 2],
        ...
        self.feed_forward = FeedForwardBlock([layer_dims[0], 1])
        ...
        self.pairwize = FeedForwardBlock([2, 1])
    ...
    def forward(self, embedding: torch.Tensor, query_ment_start: torch.Tensor, query_ment_end: torch.Tensor, **kwargs):
        ...
        query_start_logits = logits[indicies, query_ment_start]
        query_end_logits = logits[indicies, query_ment_end]
        ...
        # query_start_logits = self.feed_forward(query_start_embed)
        # query_end_logits = self.feed_forward(query_end_embed)
        query_start_end_logit = self.start_end_ff(query_start_end_embed)
        # sum to get query score
        query_score = (query_start_logits + query_end_logits + query_start_end_logit) / 3
        query_score_ex = query_score.expand(-1, max_seq_len).unsqueeze(2).expand(-1, -1, max_seq_len).clone()

        # Calculate the passage score
        # Add all start/end combinations for start/end logits
        # pass_start_logits = logits.unsqueeze(2)
        # pass_end_logits = logits.unsqueeze(2)
        pass_matrix = logits.expand(-1, -1, max_seq_len)
        # pass_end_matrix = pass_end_logits.expand(-1, -1, max_seq_len)
        pass_start_plus_end_matrix = pass_matrix + pass_matrix.transpose(1, 2)
        ...
        pass_start_end_embed = torch.cat((embed_matrix_ex, embed_matrix_ex.transpose(1, 2)), dim=3)
        pass_start_cat_end_logit = self.start_end_ff(pass_start_end_embed).squeeze()
        # sum to get passage score
        pass_score = (pass_start_plus_end_matrix + pass_start_cat_end_logit) / 3

        query_start_end_logit_ex = query_start_end_logit.expand(-1, max_seq_len).unsqueeze(2).expand(-1, -1, max_seq_len)
        pass_query_concat = torch.cat((query_start_end_logit_ex.unsqueeze(3), pass_start_cat_end_logit.unsqueeze(3)), dim=3)

        pairwize_score = self.pairwize(pass_query_concat).squeeze()
        ...
        pairwize_score[:, indices[0][:], indices[1][:]] = 0

        lambda_thresh = 0.5
        final_pairwize_score = (lambda_thresh * (pass_score + query_score_ex)) + ((1 - lambda_thresh) * pairwize_score)
